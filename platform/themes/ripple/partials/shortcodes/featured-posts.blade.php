     ...
     |---------------------------------------------------------- */
            ...
            $heroPosts = Cache::remember('home.heroPosts', 18_000, function () use ($isHome) {
                ...
                if (\App\Models\Post::whereIn('hero_order', $orders)->exists()) {
                    $sub = \App\Models\Post::select('hero_order', \DB::raw('MAX(updated_at) as max_updated'))
                ...
                return \App\Models\Post::with('categories:id,name')->latest('created_at')->take(3)->get();
            });

            $lastRecentPosts = Cache::remember('home.lastRecentPosts', 18_000, function () use ($isHome) {
                if (!$isHome) {
                    return collect();
                }
                ...
                return \App\Models\Post::with('categories:id,name')->latest('created_at')->skip(3)->take(4)->get();
            });
        @endphp
...
<div class="d-none d-lg-block col-12 col-lg-3 mx-0 px-0">
