<div class="container">
    @foreach (['Fiorentina Initial Lineup', 'Fiorentina Subs'] as $category)
        ...
        <div class="row">
            @php
                $memberPolls = auth('member')->check()
                    ? auth('member')
                        ->user()
                        ->polls()
                        ->whereIn('match_lineups_id', collect($lineup[$category])->pluck('id')->toArray())
                        ->pluck('value', 'match_lineups_id')
                    : collect();
            @endphp
                    ...
                    <div class="playerpoll-card d-flex align-items-center p-3 border rounded">
                        <div class="d-flex flex-column mr-3">
                            ...
                            <div class="d-block text-small">
                                <span
                                    class="d-block text-dark mb-1 avg-txt-{{ $player->id }}">{{ $rateInfo['average'] . ' of ' . $rateInfo['max'] }}</span>
                                <span
                                    class="d-block text-dark mb-1 count-txt-{{ $player->id }}">{{ $rateInfo['count'] . ' Polls' }}</span>
                            </div>
                        </div>
                        <div class="player-info">
                            <p class="mb-1">{{ $player->player_full_name }}</p>
                            <div class="stars"
                                @if ($memberPolls->has($player->id)) data-default="{{ $memberPolls[$player->id] }}" @endif
                                data-player-id="{{ $player->id }}"
                                data-vote-url="{{ route('polls.store', $player->id) }}">
                                @for ($i = 1; $i <= 10; $i++)
                                    <span class="star @if ($memberPolls->has($player->id) && $memberPolls->get($player->id) >= $i) selected @endif"
                                        data-value="{{ $i }}">☆</span>
                                @endfor
                            </div>
                        </div>
        ...
        parentElement.querySelectorAll('.star').forEach((starElement) => {
            ...
            starElement.addEventListener("click", (event) => {
                ...
                axios.post(parentElement.getAttribute('data-vote-url'), {
                    ...
                    document.querySelector(`.avg-txt-${playerId}`).innerText =
                        `${response.data.rate_info.average} of ${response.data.rate_info.max}`
                    document.querySelector(`.count-txt-${playerId}`).innerText =
                        `${response.data.rate_info.count} Polls`
