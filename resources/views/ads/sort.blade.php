                    ...
                    <table class="table table-striped align-middle">
                        ...
                        <tbody>
                            @forelse ($imageItems as $img)
                                @php
                                    // Robust URL resolver: accepts absolute URLs or storage keys
                                    $src = preg_match('~^https?://~i', $img->image_url ?? '')
                                        ? $img->image_url
                                        : (try { Storage::disk('wasabi')->url($img->image_url); } catch(\Throwable $e) { $img->image_url; });

                                    // Fields present on ad_group_images (or equivalent)
                                ...
                                <tr>
                                    <td>
                                        <img src="{{ $src }}" alt="" style="width:150px;height:90px;object-fit:cover;border:1px solid rgba(0,0,0,.1)">
                                    </td>
                                    <td>
                                        <input type="url"
                                               name="image_links[{{ $img->id }}]"
                                               class="form-control"
                                    ...
                                    <td>
                                        <div class="d-flex align-items-center">
                                            <input type="range"
                                                   class="form-range me-2 img-weight-slider"
                        ...
                        @forelse ($ads as $ad)
                            @php
                                ...
                                $resolve = function ($path) {
                                    ...
                                    try { return Storage::disk('wasabi')->url($path); }
